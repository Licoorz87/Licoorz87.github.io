...
function loadOrder() {
    ...
    for (let segment of dataSplit) {
        if (segment) {
            ...
            minihtml = `<section class="orders__item" id="${timeBase62}"><div class="orders__item--divisor data"><span>${dateFormated["week"]}, ${dateFormated["day"]} ${dateFormated["month"]}, ${dateFormated["year"]}</span><span>${hourFormated}</span></div><br><br><div class="orders__item--divisor grid"><span>Subtotal</span><span class="right">$</span><span class="right">${(Math.floor(subTotal * 100) / 100).toFixed(2).replace(".", ",")}</span></div><div class="orders__item--divisor grid"><span>Comissão</span><span class="right">$</span><span class="right">${(Math.floor(totalCommission * 100) / 100).toFixed(2).replace(".", ",")}</span></div><div class="orders__item--divisor grid"><span>Total</span><span class="right">$</span><span class="right">${(Math.floor(total * 100) / 100).toFixed(2).replace(".", ",")}</span></div><br><div class="orders__item--divisor payment"><section>`;
...
function initializeOrderEvents() {
    ...
    orderElements.trash.forEach((element) => {
        element.addEventListener("click", async function () {
            ...
            if (await confirmAlert("Deseja realmente apagar essa comanda?")) {
